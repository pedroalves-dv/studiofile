...
import { customerUpdateProfile, customerUpdatePassword } from '@/lib/shopify/auth'
...
export function SettingsForm({ customer }: SettingsFormProps) {
  ...
  function handlePasswordSubmit(e: React.FormEvent) {
    ...
    startPasswordTransition(async () => {
      const result = await customerUpdatePassword(customer.email, currentPassword, newPassword)
      if (result.success) {
  ...
  return (
    <div>
      {/* Profile section */}
      <section>
        <h2 className="font-display text-2xl text-ink mb-6">Profile</h2>
        <form onSubmit={handleProfileSubmit} className="flex flex-col gap-5 max-w-md">
          ...
          {profileSuccess && (
            <p className="text-sm text-success border border-success/30 bg-success/5 px-4 py-3">
              Profile updated.
            </p>
          )}
